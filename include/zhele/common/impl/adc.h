...
namespace Zhele::Private
{
    ...
    bool ADC_TEMPLATE_QUALIFIER::VerifyReady(unsigned readyMask)
    {
        ...
        while ((_Regs()->CR & readyMask) && --timeout)
        {}
    ...
    ADC_TEMPLATE_ARGS
    unsigned ADC_TEMPLATE_QUALIFIER::ConvertionTimeCycles(uint8_t channel)   // Correct G4
    {
        ...
        static const uint16_t sampleTimes[] = {2, 6, 12, 24, 46, 91, 246, 639};
    ...
    void ADC_TEMPLATE_QUALIFIER::Init(ADC_TEMPLATE_QUALIFIER::Reference reference)
    {
        ...
        _Regs()->IER  = ADC_IER_EOSIE | ADC_IER_JEOSIE;
        _Regs()->CFGR = ADC_CFGR_EXTSEL | ADC_CFGR_EXTEN;
    ...
    void ADC_TEMPLATE_QUALIFIER::Disable()
    {
        ...
        while (_Regs()->CR & ADC_CR_ADSTP)
        {
        }

        _Regs()->CR |= ADC_CR_JADSTP;
        while (_Regs()->CR & ADC_CR_JADSTP)
        {
        }
    ...
    {
        _Regs()->JSQR = (_Regs()->JSQR & ~(ADC_JSQR_JEXTSEL | ADC_JSQR_JEXTEN)) | ((trigger & 0x07) << ADC_JSQR_JEXTSEL_Pos) | (mode << ADC_JSQR_JEXTEN_Pos);
    }
    ...
    bool ADC_TEMPLATE_QUALIFIER::StartInjected(const uint8_t *channels, uint16_t *data, uint8_t count, AdcCallbackType callback)
    {
        ...
        if (!VerifyReady(ADC_CR_JADSTART))
        {
            _adcData.error = AdcError::HardwareError;
            return false;
        }

        _Regs()->CFGR |= ADC_CFGR_JDISCEN;
        ...
        _Regs()->JSQR |= ADC_JSQR_JEXTEN | ADC_JSQR_JEXTSEL;
        _Regs()->CR |= ADC_CR_JADSTART;
    ...
    bool ADC_TEMPLATE_QUALIFIER::ReadInjected(const uint8_t *channels, uint16_t *data, uint8_t count)
    {
        ...
        if (alreadyStarted || StartInjected(channels, data, count, 0))
        {
            ...
            if ((status & ADC_ISR_JEOC) == 0 || !timeout)
            {
                _adcData.error = AdcError::HardwareError;
            }
            else
    ...
    void ADC_TEMPLATE_QUALIFIER::DmaHandler(void *data, size_t size, bool success)
    {
        ...
        _Regs()->CFGR &= ~(ADC_CFGR_DMAEN | ADC_CFGR_CONT);
        _Regs()->CR |= ADC_CR_ADSTP;
        _Regs()->ISR |= ADC_ISR_EOC;
    ...
    void ADC_TEMPLATE_QUALIFIER::IrqHandler()
    {
        ...
        // reset all flags
        _Regs()->ISR |= ADC_CR_JADSTP;
        _Regs()->ISR |= ADC_ISR_JEOC;
    ...
    {
        _Regs()->CR2 = (_Regs()->CFGR & ~(ADC_CFGR_EXTSEL | ADC_CFGR_EXTEN)) | ((trigger & 0x0f) << ADC_CFGR_EXTSEL) | (mode << ADC_CFGR_EXTEN_Pos);
    }
    ...
    bool ADC_TEMPLATE_QUALIFIER::StartRegular(std::initializer_list<uint8_t> channels, uint16_t *dataBuffer, uint16_t scanCount, uint8_t discontinuous)
    {
        ...
        uint32_t controlReg = _Regs()->CFGR;
        controlReg &= ~(ADC_CFGR_DISCEN | ADC_CFGR_DISCNUM);
        if(discontinuous > 0)
        {
            controlReg |= ADC_CFGR_DISCEN;
            controlReg |= (discontinuous - 1) << ADC_CFGR_DISCNUM_Pos;
        }
        else
        {
